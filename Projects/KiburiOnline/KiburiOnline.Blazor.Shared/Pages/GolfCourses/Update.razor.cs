...
namespace KiburiOnline.Blazor.Shared.Pages.GolfCourses
{
    ...
    public partial class Update
    {
        ...
        /// </summary>
        [Parameter] public string GolfCourseId { get; set; } = null!;
        ...
        {
            var updateResult = await GolfCoursesService.UpdateAsync(_golfCourse.ToDto());
            updateResult.ProcessResponseForDisplay(SnackBar, async () =>
            {
                    ...
                    SnackBar.Add($"Action Successful. Golf Course \"{_golfCourse.Name}\" was successfully updated.", Severity.Success);
            });
        }
        ...
        private async Task RemoveGalleryImage(string url)
        {
            if (await DialogService.ConfirmAction("Are you sure you want to remove this lodging from this application?"))
            {
                ...
                var imageRemovalResult = await GolfCoursesService.RemoveImage(toRemove);
        ...
        private async Task ClearAsync()
        {
            ...
            //_golfCourse.VideosToUpload.Clear();
            ClearDragClass();
        }
        ...
        private async Task Upload()
        {
            ...
            await Parallel.ForEachAsync(_videosToUpload.ToList(), async (file, cancellationToken) =>
            {
                ...
                if (response.Succeeded)
                {
                    ...
                    var additionResult = await GolfCoursesService.AddVideo(new AddEntityVideoRequest() { EntityId = _golfCourse.GolfCourseId, VideoId = response.Data.VideoId });
                    if (!additionResult.Succeeded) SnackBar.AddErrors(additionResult.Messages);
        ...
        {
            var latLng = new LatLngLiteral(_golfCourse.Lat, _golfCourse.Lng);
        ...
        {
            var result = await GolfCoursesService.GolfCourseAsync(GolfCourseId);
            _golfCourse = new GolfCourseViewModel(result.Data);
            _imageSource = _golfCourse.Images?.FirstOrDefault(c => c.ImageType == UploadType.Cover) is null ? "_content/Accomodation.Blazor/images/NoImage.jpg" : $"{Configuration["ApiConfiguration:BaseApiAddress"]}\\{_golfCourse.Images?.FirstOrDefault(c => c.ImageType == UploadType.Cover)?.RelativePath?.TrimStart('/')}";
            ...
            {
                Zoom = 13,
                Center = new LatLngLiteral(_golfCourse.Lat, _golfCourse.Lng),
