...
namespace PayFast
{
    ...
    public class PayFastSubscription : PayFastApiBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the PayFastSubscription class using the specified PayFast settings.
        /// </summary>
        /// <param name="payfastSettings">The PayFastSettings object containing configuration values required to initialize the subscription. Cannot
        /// be null.</param>
        ...
        {
            return await Pause(token: token, cycles: 1, testing: testing);
        }
        ...
        public async Task<AdhocResult> Pause(string token, int cycles, bool testing = false)
        {
            using (var httpClient = GetClient())
            {
                ...
                var parameterValue = GenerateSignature(httpClient, incommingParameters.ToArray());
        ...
        /// <param name="testing">Pass in true to test against the sandbox. This parameter, when true appends the required '?testing=true' value to the generated query string.</param>
        /// <exception cref = "ApiResponseException"> Thrown when the returned StatusCode != HttpStatusCode.OK (200)</exception>
        public async Task<UpdateResponse> Update(string token, int? cycles = null, PayfastBillingFrequency? frequency = null, DateTime? run_date = null, int? amount = null, bool testing = false)
        {
            using (var httpClient = GetClient())
            {
