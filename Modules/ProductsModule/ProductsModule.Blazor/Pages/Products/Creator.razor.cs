...
public partial class Creator
{
    ...
    private readonly List<BreadcrumbItem> _items = new()
    {
        ...
        new("Products", href: "/products", icon: Icons.Material.Filled.List),
    ...
    /// <summary>
    /// Adds the specified product data transfer object (DTO) to the collection of variants to upload.
    /// </summary>
    /// <remarks>This method is used to queue product variants for upload. Ensure that the provided <paramref
    /// name="dto"/> contains valid product data before calling this method.</remarks>
    /// <param name="dto">The product data transfer object to be added. This parameter cannot be <see langword="null"/>.</param>
    ...
    /// </summary>
    /// <remarks>If the product specified by <paramref name="dto"/> does not exist in the collection, this
    /// method does not perform any operation.</remarks>
    ...
    /// upload.
    /// </summary>
    /// <remarks>If no product variant with the specified ID exists in the collection, no action is
    /// performed.</remarks>
    ...
    /// </summary>
    /// <param name="dto">The product metadata to add. This parameter cannot be null.</param>
    ...
    /// <summary>
    /// Updates the metadata for a product in the collection.
    /// </summary>
    /// <remarks>This method replaces the metadata of an existing product in the collection with the provided
    /// <paramref name="dto"/>.  If no matching product is found, the behavior of this method is undefined.</remarks>
    /// <param name="dto">The updated product metadata to apply. The <see cref="ProductMetadataDto.ProductId"/> property must match an
    /// existing product in the collection.</param>
    ...
    /// <summary>
    /// Removes the product metadata associated with the specified metadata identifier.
    /// </summary>
    /// <remarks>If no product metadata with the specified identifier exists, the method performs no
    /// action.</remarks>
    /// <param name="metadataId">The identifier of the product metadata to remove. This value cannot be <see langword="null"/> or empty.</param>
    ...
    {
        var result = await ProductService.CreateAsync(_product.ToDto());
        result.ProcessResponseForDisplay(SnackBar, async () =>
        {
            if (!string.IsNullOrEmpty(_coverImageToUpload))
            {
                ...
                {
                    Name = $"{_product.Details.Name} CoverImage",
                ...
                foreach (var galleryImage in _galleryImageToUpload)
                {
                    ...
                    {
                        var additionResult = await ProductService.AddImage(new AddEntityImageRequest() { EntityId = _product.ProductId, ImageId = galleryUploadResult.Data.Id });
                        if (!additionResult.Succeeded) SnackBar.AddErrors(additionResult.Messages);
                    }
                }
                ...
                {
                    var createResult = await ProductService.CreateMetadata(metaData);
                    if (!createResult.Succeeded) SnackBar.AddErrors(createResult.Messages);
                }

                await Upload();
            }
            SnackBar.AddSuccess($"{_product.Details.Name} was created successfully");
    ...
    private async Task Upload()
    {
        ...
        await Parallel.ForEachAsync(_videosToUpload.ToList(), async (file, cancellationToken) =>
        {
            ...
            if (response.Succeeded)
            {
                ...
                var additionResult = await ProductService.AddVideo(new AddEntityVideoRequest() { EntityId = _product.ProductId, VideoId = response.Data.VideoId }, cancellationToken);
                if (!additionResult.Succeeded) SnackBar.AddErrors(additionResult.Messages);
  ...
  //      _product.Settings.ShopOwnerId = ShopOwnerId;
