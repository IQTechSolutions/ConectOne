...
public partial class Editor
{
    ...
    private ServiceViewModel _service = new();
    ...
    private readonly List<BreadcrumbItem> _items = new()
    {
        ...
        new("Update Product", href: null, disabled: true, icon: Icons.Material.Filled.Edit)
    };
    ...
    /// </summary>
    [Inject] public IImageProcessingService ImageProcessingService { get; set; }

    /// <summary>
    /// Gets or sets the service responsible for processing video-related operations.
    /// </summary>
    [Inject] public IVideoProcessingService VideoProcessingService { get; set; }
    ...
    private async Task RemoveGalleryImage(string imageId)
    {
        if (await DialogService.ConfirmAction("Are you sure you want to remove this lodging from this application?"))
        {
            var imageRemovalResult = await ServiceService.RemoveImage(imageId);
            imageRemovalResult.ProcessResponseForDisplay(SnackBar, () =>
            {
                _service.Images.Remove(_service.Images.FirstOrDefault(c => c.Id == imageId));
                SnackBar.AddSuccess("Image successfully removed");
            });
        }
    }
    ...
    private async Task Upload()
    {
        ...
        await Parallel.ForEachAsync(_videosToUpload.ToList(), async (file, cancellationToken) =>
        {
            ...
            if (response.Succeeded)
            {
                ...
                var additionResult = await ServiceService.AddVideo(new AddEntityVideoRequest() { EntityId = _service.Id, VideoId = response.Data.VideoId });
    ...
    private async Task RemoveVideo(string videoId)
    {
        ...
        {
            var imageRemovalResult = await ServiceService.RemoveVideo(videoId);
            imageRemovalResult.ProcessResponseForDisplay(SnackBar, () =>
            {
                _service.Videos.Remove(_service.Videos.FirstOrDefault(c => c.Id == videoId));
    ...
    {
        var result = await ServiceService.UpdateAsync(_service.ToDto());
        result.ProcessResponseForDisplay(SnackBar, async () =>
        {
            if (!string.IsNullOrEmpty(_coverImageToUpload))
            {
                ...
                {
                    Name = $"{_service.Name} Cover Image",
                ...
                {
                    if (_service.Images.Any(c => c.ImageType == UploadType.Cover))
                    {
                        var removalResult = await ServiceService.RemoveImage(_service.Images.FirstOrDefault(c => c.ImageType == UploadType.Cover).Id);
            ...
            SnackBar.AddSuccess($"{_service.Name} was updated successfully");
        });
    }
    ...
    {
        var result = await ServiceService.ServiceAsync(ProductId);
        if (result.Succeeded)
        {
            _service = new ServiceViewModel(result.Data);
            _imageSource = _service.Images?.FirstOrDefault(c => c.ImageType == UploadType.Cover)?.RelativePath ?? _imageSource;
        }
